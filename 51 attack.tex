...
If the attacker wants to make node to switch his fraudulent branch, he will need to produce higher \text{$H\!W\!D$} value. The attacker needs to mine in the original branch for a while to make itself included in the history. Therefore, when it switches to hidden branch, its \text{$H\!W\!D$} will be greater. 
...
The second approach is to post an overlap requirement between two split branches. To minimize the cost of attack, attacker will focus its hashrate to the malicious branch. The overlap requires the new branch posses a certain amount of miner overlap with the old branch. Under such a condition, in order for a branch switch to happen, one needs to satisfy not only the \text{$H\!W\!D$} condition,
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \begin{axis}[
        xlabel={Ethereum mainnet block number $n$ (million)},
        ylabel style={align=center}, ylabel={cost to perform a 51\% attack (Ethers)}\\{$cost = window * \sqrt{correlation\ rate}$},
        ymode=log,
        ymax=3000000,
        grid=major,
...
The result indicates that with \text{$H\!W\!D$} scheme, the cost of attacking the Ethereum Mainnet blockchain is increased by more than 1000 times if we set the window size to 1M or 2M blocks. In real application, the window could be shorter to speed up the process. We suggest window size is at least 100k, making the cost of attack 100x more expensive.


\section{Conclusion}
 
In this paper, we proposed an approach to increase the cost of a successful 51\% double-spending attack on Proof-of-Work types of Blockchain protocols. The proposed approach utilizes the frequency rate of miners in history blocks and calculates the total Historical Weighted Difficulty to determine if branch switch is needed. We demonstrated in three real Ethereum Mainnet scenarios that the cost of attack is increased by more than 1000 times with \text{$H\!W\!D$}-PoW scheme.
