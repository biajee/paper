...
The result indicates that with \text{$H\!W\!D$} scheme, the cost of attacking the Ethereum Mainnet blockchain is increased by more than 100 times if we set the window size to 1M or 2M blocks. In real application, the window could be shorter to speed up the process. We suggest window size is at least 100k, making the cost of attack 100x more expensive.
